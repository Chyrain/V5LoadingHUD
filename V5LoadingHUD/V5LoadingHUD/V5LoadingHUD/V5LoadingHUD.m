...
+(V5LoadingHUD*)showIn:(UIView*)view{
    [self hideIn:view];
    V5LoadingHUD *hud = [[V5LoadingHUD alloc] initWithFrame:view.bounds];
    [hud start];
    [view addSubview:hud];
    return hud;
}
...
+(V5LoadingHUD *)hideIn:(UIView *)view{
    V5LoadingHUD *hud = nil;
    for (V5LoadingHUD *subView in view.subviews) {
        if ([subView isKindOfClass:[V5LoadingHUD class]]) {
            [subView hide];
